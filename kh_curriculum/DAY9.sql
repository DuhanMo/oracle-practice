...
/*
    ...
    1) EMP001테이블의 EMP_ID 컬럼에 대해 UNIQUE INDEX 만들기 : IDX_EID
    2) EMP001 테이블의 EMP_ID가 200 미만인 새직원 정보를 삽입
*/
...
--2) EMP001 테이블의 EMP_ID가 200 미만인 새직원 정보를 삽입
INSERT INTO EMP001 VALUES (199,'테스트','881122-1234567','test@test.com','01098765432','D9','J1','S4',1000000,0.5,null,SYSDATE,NULL,DEFAULT);

SELECT * FROM EMP001;

-- * 인덱스 모니터링 : 만들어진 인덱스가 조회 시 사용되었는지 확인
-- 모니터링 할 인덱스 설정
ALTER INDEX IDX_EID MONITORING USAGE;

-- 인덱스 모니터링
SELECT * FROM V$OBJECT_USAGE;
-- V$OBJECT_USAGE : 인덱스 활용과 관련된 데이터를 수집하는 뷰 테이블
-- USED 컬럼 : 모니터링 시작 후 해당 인덱스가 사용되었는지 확인

-- (1) 인덱스 활용 x 전체 조회
SELECT * FROM EMP001;
-- > 새 직원의 정보가 테이블의 제일 마지막에 조회된다.

-- 인덱스 모니터링
SELECT * FROM V$OBJECT_USAGE;

-- (2) 인덱스 활용 0 전체 조회
SELECT * FROM EMP001 WHERE EMP_ID > '0';
